...
def train_rnn(  # noqa: C901, PLR0912, PLR0913, PLR0915
    ...
    if window_size is not None:
        ...
        def _collate_prefix(batch: list[tuple[torch.Tensor, int]]) -> tuple[torch.Tensor, torch.Tensor]:
            ...
            # If the batch contains prefixes shorter than window_size, do NOT
            # force-pad them up to window_size — this makes short prefixes
            # behave like the non-windowed (teacher-forcing) case. Only when
            # a prefix exceeds window_size (we cropped them earlier) will we
            # pad to window_size. Practically we pad to the batch max length
            # but cap it at window_size so we don't expand shorter prefixes.
            batch_max = 0 if not xs else max(int(s.numel()) for s in xs)
            target_len = min(window_size, batch_max) if window_size is not None else batch_max
            x_pad = _left_pad_stack(xs, target_len=target_len)
            return x_pad, ys
