...
# Energy measurements are disabled by default.
...
def create_energy_tracker(phase_name: str) -> EnergyPhaseTracker | None:
    """
    Create an energy tracker for this phase.

    Returns None when energy measurement is disabled or unavailable.
    """
...
def get_phase_energy_joules(tracker: EnergyPhaseTracker | None) -> float | None:
    """Return measured phase energy, or None if measurement is unavailable."""
    if tracker is None:
        return None
...
def process_neural_model(  # noqa: C901, PLR0913, PLR0915
    iteration_data: dict,
    all_metrics: dict,
    ...
    *,
    ...
    # Initialize energy tracker for training if explicitly enabled
    energy_train_tracker = create_energy_tracker(f"{display_name}_train")
    if energy_train_tracker is not None:
    ...
    # Measure energy for training only when the tracker is enabled and available
    energy_train_joules = get_phase_energy_joules(energy_train_tracker)
    ...
    # Initialize energy tracker for evaluation if explicitly enabled
    energy_eval_tracker = create_energy_tracker(f"{display_name}_eval")
    if energy_eval_tracker is not None:
    ...
    # Measure energy for evaluation only when the tracker is enabled and available
    energy_eval_joules = get_phase_energy_joules(energy_eval_tracker)
    ...
    record_model_results(
        ...
        if energy_train_joules is not None and energy_eval_joules is not None
        else None,
    )
...
def process_rl_model(
    ...
    epochs: int = 20,
) -> tuple[dict[str, Any], list[Any], float, list[Any], float, float, float, float | None, float | None]:
    """
    ...
    # Initialize energy tracker for RL training if explicitly enabled
    energy_train_tracker = create_energy_tracker("rl_train")
    if energy_train_tracker is not None:
        energy_train_tracker.start()
    ...
    # Measure energy for RL training only when the tracker is enabled and available
    energy_train_joules = get_phase_energy_joules(energy_train_tracker)

    # Evaluate on the common evaluation set (same sequences as other models)
    memory_eval_peak_start_mb = get_current_rss_mb()

    # Initialize energy tracker for RL evaluation if explicitly enabled
    energy_eval_tracker = create_energy_tracker("rl_eval")
    if energy_eval_tracker is not None:
        energy_eval_tracker.start()
    ...
    # Measure energy for RL evaluation only when the tracker is enabled and available
    energy_eval_joules = get_phase_energy_joules(energy_eval_tracker)
...
MEASURE_ENERGY = bool(run_config.get("energy", {}).get("enabled", False))
...
for iteration in range(N_ITERATIONS):
    ...
    while strategies:
        ...
        # Initialize energy tracker for mining/training if explicitly enabled
        energy_train_tracker = create_energy_tracker(f"{strategy_name}_train")
        if energy_train_tracker is not None:
            energy_train_tracker.start()
        ...
        # Measure energy for miner training only when the tracker is enabled and available
        energy_train_joules = get_phase_energy_joules(energy_train_tracker)
        ...
        # Initialize energy tracker for evaluation if explicitly enabled
        energy_eval_tracker = create_energy_tracker(f"{strategy_name}_eval")
        if energy_eval_tracker is not None:
            energy_eval_tracker.start()
        ...
        # Measure energy for miner evaluation only when the tracker is enabled and available
        energy_eval_joules = get_phase_energy_joules(energy_eval_tracker)
        ...
        if RL_TRAINING:
            for w in [None, *NN_WINDOW_RANGE] : # Include both no-window and windowed variants
                ...
                record_model_results(
                    ...
                    if rl_energy_train_joules is not None and rl_energy_eval_joules is not None
                    else None,
                )
